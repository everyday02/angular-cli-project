...
export class MapComponent implements OnInit {
    ...
    ngOnInit() {
            // 监听事件应该放到ngOnInit里
            this._pubsub.confirm.subscribe(data => this.confirmDelete());
            this.reload();
        }
    ...
    reload() {
        this.http.request('http://jsonplaceholder.typicode.com/userss').map(res => res.json()).subscribe(data => {
            this.users = data;
            console.log(data);
        }, err => console.log(err), () => console.log('Get Users Complete'));
    }
    confirmDelete() {
        let req = {
            name: "jiangbo"
        }
        this.http.delete('http://jsonplaceholder.typicode.com/userss', { body: JSON.stringify(req) }).map(res => res.json()).subscribe(data => {
            this.users = data;
            console.log(data);
        }, err => console.log(err), () => console.log('Get Users Complete'));
    }
